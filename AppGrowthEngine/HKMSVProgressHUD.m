...
@implementation HKMSVProgressHUD

@synthesize maskType, fadeOutTimer, stringLabel, imageView, spinnerView, visibleKeyboardHeight;

static HKMSVProgressHUD *sharedView = nil;

+ (HKMSVProgressHUD*)sharedView {
	
	if(sharedView == nil)
		sharedView = [[HKMSVProgressHUD alloc] initWithFrame:CGRectZero];
	
	return sharedView;
}

+ (void)setStatus:(NSString *)string {
	[[HKMSVProgressHUD sharedView] setStatus:string];
}
...
+ (void)show {
	[HKMSVProgressHUD showInView:nil status:nil];
}

+ (void)showInView:(UIView*)view {
	[HKMSVProgressHUD showInView:view status:nil];
}

+ (void)showInView:(UIView*)view status:(NSString*)string {
	[HKMSVProgressHUD showInView:view status:string networkIndicator:SVProgressHUDShowNetworkIndicator];
}

+ (void)showInView:(UIView*)view status:(NSString*)string networkIndicator:(BOOL)show {
	[HKMSVProgressHUD showInView:view status:string networkIndicator:show posY:-1];
}

+ (void)showInView:(UIView*)view status:(NSString*)string networkIndicator:(BOOL)show posY:(CGFloat)posY {
    [HKMSVProgressHUD showInView:view status:string networkIndicator:show posY:posY maskType:SVProgressHUDMaskTypeNone];
}

+ (void)showWithStatus:(NSString *)status {
    [HKMSVProgressHUD showInView:nil status:status];
}

+ (void)showWithMaskType:(SVProgressHUDMaskType)maskType {
    [HKMSVProgressHUD showInView:nil status:nil networkIndicator:SVProgressHUDShowNetworkIndicator posY:-1 maskType:maskType];
}

+ (void)showWithStatus:(NSString*)status maskType:(SVProgressHUDMaskType)maskType {
    [HKMSVProgressHUD showInView:nil status:status networkIndicator:SVProgressHUDShowNetworkIndicator posY:-1 maskType:maskType];
}

+ (void)showWithStatus:(NSString *)status networkIndicator:(BOOL)show {
    [HKMSVProgressHUD showInView:nil status:status networkIndicator:show];
}

+ (void)showWithMaskType:(SVProgressHUDMaskType)maskType networkIndicator:(BOOL)show {
    [HKMSVProgressHUD showInView:nil status:nil networkIndicator:show posY:-1 maskType:maskType];
}

+ (void)showWithStatus:(NSString*)status maskType:(SVProgressHUDMaskType)maskType networkIndicator:(BOOL)show {
    [HKMSVProgressHUD showInView:nil status:status networkIndicator:show posY:-1 maskType:maskType];
}

+ (void)showSuccessWithStatus:(NSString *)string {
    [HKMSVProgressHUD show];
    [HKMSVProgressHUD dismissWithSuccess:string afterDelay:1];
}
...
+ (void)showInView:(UIView*)view status:(NSString*)string networkIndicator:(BOOL)show posY:(CGFloat)posY maskType:(SVProgressHUDMaskType)hudMaskType {
    [[HKMSVProgressHUD sharedView] showInView:view status:string networkIndicator:show posY:posY maskType:hudMaskType];
}
...
+ (void)dismiss {
	[[HKMSVProgressHUD sharedView] dismiss];
}

+ (void)dismissWithSuccess:(NSString*)successString {
	[[HKMSVProgressHUD sharedView] dismissWithStatus:successString error:NO];
}

+ (void)dismissWithSuccess:(NSString *)successString afterDelay:(NSTimeInterval)seconds {
    [[HKMSVProgressHUD sharedView] dismissWithStatus:successString error:NO afterDelay:seconds];
}

+ (void)dismissWithError:(NSString*)errorString {
	[[HKMSVProgressHUD sharedView] dismissWithStatus:errorString error:YES];
}

+ (void)dismissWithError:(NSString *)errorString afterDelay:(NSTimeInterval)seconds {
    [[HKMSVProgressHUD sharedView] dismissWithStatus:errorString error:YES afterDelay:seconds];
}
...
- (void)setStatus:(NSString *)string {
    ...
    NSLog(@"HUDView center 1 is %@ %f %f", string, _hudView.center.x, _hudView.center.y);
    ...
    NSLog(@"HUDView center 2 is %@, %f %f", string, _hudView.center.x, _hudView.center.y);
...
- (void)dismissWithStatus:(NSString *)string error:(BOOL)error afterDelay:(NSTimeInterval)seconds {
	...
	if(error)
		self.imageView.image = [UIImage imageNamed:@"HKMSVProgressHUD.bundle/error.png"];
	else
		self.imageView.image = [UIImage imageNamed:@"HKMSVProgressHUD.bundle/success.png"];
